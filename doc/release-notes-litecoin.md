Terracash Core version *0.15.1* is now available from:

  <https://download.terracash.org/terracash-0.15.1/>
...
A number of changes to the way Terracash Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 terra per kB (0.00001 TRCH). This is to prepare for dropping the minimum transaction fee to 0.1 terra per kB (0.0001 TRCH) in the next release.
