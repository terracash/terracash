...
*Setup instructions for a Gitian build of Terracash Core using a Debian VM or physical system.*
...
- [Building Terracash Core](#building-terracash-core)
...
Clone the git repositories for terracash and Gitian.
...
in the terracash repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Terracash Core
----------------

To build Terracash Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the terracash repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/terracash-project/terracash
...
For example:
```bash
URL=https://github.com/thrasher-/terracash.git
...
and the terracash git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../terracash/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/terracash
SIGPATH=/some/root/path/terracash-detached-sigs

./bin/gbuild --url terracash=${BTCPATH},signature=${SIGPATH} ../terracash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/terracash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/terracash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/terracash-osx-build.assert
```
...
[terracash-project/gitian.sigs.ltc](https://github.com/terracash-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
